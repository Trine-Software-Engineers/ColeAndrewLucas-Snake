...
void GameOverBox()
{
    QString scoreMsg;
    if(score > highscore) {
        scoreMsg = "New High Score!\nYour score of " + QString::number(score) + " beat the previous highscore of " + QString::number(highscore) + "   Congratulations!   \nDo you want to play again?";
        highscore = score;
    } else {
        scoreMsg = "High Score: " + QString::number(highscore) + "\nYour snake was: " + QString::number(score) + " segments long!\nThank you for playing. Do you want to play again?";
    }
    QMessageBox msgBox;
    msgBox.setWindowTitle("Game Over!");
    msgBox.setText(scoreMsg);
    msgBox.setStandardButtons(QMessageBox::Yes);
    msgBox.addButton(QMessageBox::No);
    msgBox.setDefaultButton(QMessageBox::No);
